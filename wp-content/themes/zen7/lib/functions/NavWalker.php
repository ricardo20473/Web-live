...
add_filter( 'wp_setup_nav_menu_item', 'zen_add_custom_nav_fields' );

// save menu custom fields
add_action( 'wp_update_nav_menu_item', 'zen_update_custom_nav_fields', 10, 3 );

// edit menu walker
add_filter( 'wp_edit_nav_menu_walker', 'zen_edit_walker', 10, 2 );


function zen_add_custom_nav_fields( $menu_item ) {
...
function zen_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {

    $check = array('icon', 'color', 'subtitle', 'megamenu', 'column_title');
...
function zen_edit_walker( $walker, $menu_id ) {
...
class ZenControllerExtensionNavWalker extends Walker_Nav_Menu {
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if ($depth == 1 && $item->column_title == 'on') {
            ...
            if($depth == 0 && $item->icon != '') { $item_output .= '<span class="entypo" data-file="'.$item->color.'"><i class="'.$item->icon.' icon"></i></span>';}
